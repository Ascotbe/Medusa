        ...
        try:
            ...
            if uid != None:  # 查到了UID
                ...
                if int(number_of_pages)<0:
                    ...
                    result=NistData().BulkQuery(number_of_pages=int(number_of_pages))#获取数据
                    number = NistData().StatisticalData()  # 统计的个数
        ...
        try:
            ...
            if uid != None:  # 查到了UID
                ...
                if int(number_of_pages)<1:
                    ...
                    number = NistData().SearchStatistics(key=key, severity=severity)  # 查询漏洞总数
                    return JsonResponse({'message': data,'number': number, 'code': 200, })
            else:
                return JsonResponse({'message': "小宝贝这是非法查询哦(๑•̀ㅂ•́)و✧", 'code': 403, })
        except Exception as e:
            ErrorLog().Write(e)
