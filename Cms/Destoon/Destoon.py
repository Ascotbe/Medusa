...
def Main(Url,FileName,Values,ProxyIp):
    ...
    Medusa = [DestoonFrontDeskGetshellVulnerability.medusa(Url,RandomAgent,ProxyIp),
              DestoonSQLInjectionVulnerability1.medusa(Url, RandomAgent, ProxyIp),
              DestoonSQLInjectionVulnerability2.medusa(Url, RandomAgent, ProxyIp),
              DestoonSQLInjectionVulnerability3.medusa(Url, RandomAgent, ProxyIp),
              ]
    try:
        for i in tqdm(Medusa, ascii=True, desc="Destoon plugin progress"):
