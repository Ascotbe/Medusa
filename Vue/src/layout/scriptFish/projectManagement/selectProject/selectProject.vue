<template>
  <div class="selectProject">
    ...
    >
      <a-col :xs="{ span: 24 }">
        <a-table :columns="columns" :data-source="data" :expand="expand" bordered :scroll="{ x: 1600 }" >
          <a-tabs
            slot="expandedRowRender"
            slot-scope="record"
            :default-active-key="record.request_method"
            @change="callback"
          >
  ...
  methods: {
    handleQuery_script_project_data() {
      if (this.$store.state.project_associated_file_name != "null") {
        let params = {
          project_associated_file_name: this.$store.state.project_associated_file_name,
...
.selectProject {
  ...
  .selectProject_bg {
    ...
    /*定义整体的宽度*/
  .selectProject_bg /deep/.ant-table-body::-webkit-scrollbar {
    height: 3px;
  }

  /*定义滚动条轨道*/
  .selectProject_bg /deep/.ant-table-body::-webkit-scrollbar-track {
    border-radius: 5px;
  }

  /*定义滑块*/
  .selectProject_bg /deep/.ant-table-body::-webkit-scrollbar-thumb {
    border-radius: 5px;
