    ...
    if request.method == "POST":
        try:
            ...
            if uid != None:  # 查到了UID
                ...
                dns_key=log_key+"."+domain_name_system_address#拼接获取该用户的DNSLOG完整值
                result = DomainNameSystemLog().Query2DNS(key=dns_key,
                    number_of_pages=int(number_of_pages))  # 对解析记录进行查询
                number = DomainNameSystemLog().Statistical2DNS(key=dns_key)  # 统计的个数
        ...
        try:
            ...
            if uid != None:  # 查到了UID
                ...
                result = DomainNameSystemLog().Query2HTTP(
                    number_of_pages=int(number_of_pages))  # 对解析记录进行查询
                number = DomainNameSystemLog().Statistical2HTTP()  # 统计的个数
                return JsonResponse({'message': result,'number':number,  'code': 200, })
