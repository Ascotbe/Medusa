...
def HomepageDefaultData(request):#用户登录成功后跳转的首页，默认数据
    ...
    if request.method == "POST":
        try:
            ...
            ErrorLog().Write("Web_BasicFunctions_Home_HomepageDefaultData(def)", e)
...
def HomepageVulnerabilityDistributiontData(request):#用户登录成功后跳转的首页，漏洞分布数据
    ...
    if request.method == "POST":
        try:
            ...
            ErrorLog().Write("Web_BasicFunctions_Home_HomepageVulnerabilityDistributiontData(def)", e)
            return JsonResponse({'message': '呐呐呐！莎酱被玩坏啦(>^ω^<)', 'code': 169, })
    else:
        return JsonResponse({'message': '请使用Post请求', 'code': 500, })
...
def HomepageGithubMonitorData(request):#用户登录成功后跳转的首页，Github监控数据
    ...
    if request.method == "POST":
        try:
            ...
            ErrorLog().Write("Web_BasicFunctions_Home_HomepageGithubMonitorData(def)", e)
            return JsonResponse({'message': '呐呐呐！莎酱被玩坏啦(>^ω^<)', 'code': 169, })
    else:
        return JsonResponse({'message': '请使用Post请求', 'code': 500, })