...
class VulnerabilityInfo(object):
    def __init__(self,Medusa):
        ...
        self.info['algroup'] = "Sftp"  # 插件名称
        self.info['name'] ='Sftp信息泄露漏洞' #漏洞名称
        self.info['affects'] = "sftp"  # 漏洞组件
...
def medusa(Url,RandomAgent,ProxyIp):
    ...
    for payload in list:
        PayloadUrl =scheme + '://' + url + ':' + str(port)+payload+'/sftp-config.json'
        try:
            s = requests.session()
            if ProxyIp!=None:
                proxies = {
