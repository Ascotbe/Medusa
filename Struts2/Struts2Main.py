...
def Main(url):
    try:
        vul=Struts2.S2_001.S2_001_Main(url)
    ...
    try:
        vul =Struts2.S2_007.S2_007_Main(url)
        filename = time.strftime("%Y-%m-%d", time.localtime())  # 获取日期作为文件
        filenames = filename + ".txt"
        with open(filenames, 'a') as f:  # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！
            f.write(vul + "\n")
    ...
    try:
        vul =Struts2.S2_012.S2_012_Main(url)
        filename = time.strftime("%Y-%m-%d", time.localtime())  # 获取日期作为文件
        filenames = filename + ".txt"
        with open(filenames, 'a') as f:  # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！
            f.write(vul + "\n")
    ...
    try:
        vul =Struts2.S2_013.S2_013_Main(url)
        filename = time.strftime("%Y-%m-%d", time.localtime())  # 获取日期作为文件
        filenames = filename + ".txt"
        with open(filenames, 'a') as f:  # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！
            f.write(vul + "\n")
    ...
    try:
        vul =Struts2.S2_016.S2_016_Main(url)
        filename = time.strftime("%Y-%m-%d", time.localtime())  # 获取日期作为文件
        filenames = filename + ".txt"
        with open(filenames, 'a') as f:  # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！
            f.write(vul + "\n")
    ...
    try:
        vul =Struts2.S2_052.S2_052_Main(url)
        filename = time.strftime("%Y-%m-%d", time.localtime())  # 获取日期作为文件
        filenames = filename + ".txt"
        with open(filenames, 'a') as f:  # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！
            f.write(vul + "\n")
    ...
    try:
        vul =Struts2.S2_053.S2_053_Main(url)
        filename = time.strftime("%Y-%m-%d", time.localtime())  # 获取日期作为文件
        filenames = filename + ".txt"
        with open(filenames, 'a') as f:  # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！
            f.write(vul + "\n")
    ...
    try:
        vul =Struts2.S2_057.S2_057_Main(url)
        filename = time.strftime("%Y-%m-%d", time.localtime())  # 获取日期作为文件
        filenames = filename + ".txt"
        with open(filenames, 'a') as f:  # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！
            f.write(vul + "\n")
