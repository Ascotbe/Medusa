...
# external modules
from Modules.InformationDetector.subbrute import subbrute
import dns.resolver
import requests
...
class portscan():
    ...
    def run(self):
        for subdomain in self.subdomains:
         ...
         #目标     线程数  保存路径  不输出
def main(domain, savefile, silent,subbrutes):
    threads="30"#线程数
    bruteforce_list = set()
    search_list = set()
    ...
    supported_engines = {'baidu': BaiduEnum,#支持的引擎
                         'yahoo': YahooEnum,
                         'google': GoogleEnum,
                         'bing': BingEnum,
                         'ask': AskEnum,
    ...
    # 启动引擎枚举
    try:
        enums = [enum(domain, [], q=subdomains_queue, silent=silent, verbose=False) for enum in chosenEnums]
        for enum in enums:
            enum.start()
        for enum in enums:
            enum.join()
    ...
    if subbrutes:
        ...
        output = False
        json_output = False
        bruteforce_list = subbrute.print_target(parsed_domain.netloc, record_type, subs, resolvers, process_count, output, json_output, search_list, verbose=False)
